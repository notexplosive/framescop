...
Timeline.draw = function(self)
    local currentFrontierPosition =
        love.graphics.getWidth() * (self.film.cachedFrontier - self.film.playhead) / self.film.totalFrames
...
Timeline.isHover = function(self)
    ...
    if mx > self.x - self.width / 2 and mx < self.x + self.width / 2 and my > love.graphics.getHeight() - self.height then
        return true
    end
    return false
end
